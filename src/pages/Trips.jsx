            ...
            <div className="space-y-4 mb-6">
                <div>
                    ...
                    <input
                        ...
                        className="w-full bg-gray-950 border border-white/5 rounded-2xl p-4 outline-none focus:border-emerald-500 transition-colors font-bold text-white"
                    />
                </div>

                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-1.5 ml-2">Start Date</p>
                        <input
                            type="date"
                            value={newStart}
                            onChange={(e) => setNewStart(e.target.value)}
                            className="w-full bg-gray-950 border border-white/5 rounded-2xl p-4 text-xs font-bold text-white outline-none focus:border-emerald-500"
                        />
                    </div>
                    <div>
                        <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-1.5 ml-2">End Date (Opt)</p>
                        <input
                            type="date"
                            value={newEnd}
              ...
              >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                          ...
                          <p className="text-[9px] font-black text-gray-600 uppercase tracking-widest">
                            {formatDate(trip.startDate) || "Anytime"}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-950/40 rounded-[1.8rem] p-5 border border-white/5">
                    <p className="text-[8px] font-black text-gray-600 uppercase tracking-widest mb-1.5">Spending</p>
          ...
          <div className="fixed inset-0 z-[110] flex flex-col justify-end bg-black/80 backdrop-blur-sm">
            ...
            <div className="bg-gray-900 w-full rounded-t-[2.5rem] border-t border-gray-800 p-8 pb-12 shadow-2xl">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-sm font-black uppercase tracking-widest text-indigo-400">Edit Journey</h2>
                    <div className="flex gap-2">
                        <button
                            onClick={(e) => handleDeleteTrip(editingTrip.id, e)}
                            className="p-3 bg-red-500/10 rounded-2xl text-red-500 border border-red-500/10"
                        >
                            <Trash2 size={20} />
                        </button>
                        <button onClick={() => setEditingTrip(null)} className="p-3 bg-gray-800 rounded-2xl text-gray-400 border border-gray-700">
                            <X size={20} />
                        </button>
                    </div>
                </div>
                ...
                <div className="space-y-6 mb-10">
                    <div>
                        <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-2 ml-2">Destination Name</p>
                        <input
                            type="text"
                            value={editName}
                            onChange={(e) => setEditName(e.target.value)}
                            className="w-full bg-gray-950 border border-white/5 rounded-2xl p-5 outline-none focus:border-indigo-500 transition-colors font-black text-white text-lg"
                        />
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-2 ml-2">Departure</p>
                            <input
                                type="date"
                                value={editStart}
                                onChange={(e) => setEditStart(e.target.value)}
                                className="w-full bg-gray-950 border border-white/5 rounded-2xl p-5 text-xs font-black text-white outline-none focus:border-indigo-500"
                            />
                        </div>
                        <div>
                            <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-2 ml-2">Return (Optional)</p>
                            <input
                                type="date"
                                value={editEnd}
                ...
                <button
                    ...
                    className="w-full bg-indigo-600 text-white font-black py-5 rounded-3xl uppercase tracking-[0.3em] text-xs shadow-xl shadow-indigo-600/20 active:scale-95 transition-all flex items-center justify-center gap-3"
                >
                    <Save size={18} /> Update Trip Details
                </button>
            </div>
          </div>
      )}

      {/* Transaction Modal specialized for the selected trip */}
      <TransactionModal
